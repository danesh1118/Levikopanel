...
const SYSTEM_DEFAULTS = {
    ...
    isPaused: false,
    silentAlerts: false,
    githubRepo: "danesh1118/Levikopanel",
...
function generateApiKey(name) {
    ...
    const raw = `leviko_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
    const key = raw;
    return {
        id,
        ...
        try {
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    const dashboardUrl = env.DASHBOARD_URL || 'https://raw.githubusercontent.com/danesh1118/Levikopanel/main/dashboard.html';
                    try {
                ...
                if (reqPath === routes.data) {
                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        if (isValidUser) {
                            const subscriptionUrl = env.SUBSCRIPTION_URL || 'https://raw.githubusercontent.com/danesh1118/Levikopanel/main/subscription.html';
                            try {
    ...
    async scheduled(event, env, ctx) {
        try {
            ...
            if (sysConfig.autoUpdate && sysConfig.cfAccountId && sysConfig.cfApiToken && sysConfig.cfWorkerName) {
                const repo = (sysConfig.githubRepo || "danesh1118/Levikopanel")
...
function obfuscateCode(srcText) {
    ...
    cleanCode = cleanCode.replace(/export\s+default\s+/g, "const _0xLEVIKOModule = ");
    cleanCode += "\nreturn _0xLEVIKOModule;";
    ...
    const finalLoaderCode =
        rawImportsStr +
        "\n\n" +
        "// LEVIKO Panel - Obfuscated Loader Context (v2.5.4.2 Optimized)\n" +
        'const _0xLEVIKOPayload = "' +
        hexOutput +
        '";\n' +
        "const _0xLEVIKOKey = " +
        randKey +
        ";\n\n" +
        "const _0xLEVIKOBytes = new Uint8Array((_0xLEVIKOPayload.match(/.{1,2}/g) || []).map(x => parseInt(x, 16) ^ _0xLEVIKOKey));\n" +
        "const _0xLEVIKOCode = new TextDecoder().decode(_0xLEVIKOBytes);\n" +
        "const _0xLEVIKORuntime = new Function(" +
        bindingNames.map((name) => '"' + name + '"').join(", ") +
        ", _0xLEVIKOCode)(" +
        bindingNames.join(", ") +
        ");\n\n" +
        "export default _0xLEVIKORuntime;";
